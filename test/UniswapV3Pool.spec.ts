...
describe('UniswapV3Pool', () => {
  ...
  describe('#initialize', () => {
    ...
    // DEPRECATED: Oracle/TWAP functionality removed - observations are not written
    it('observations slot is zero after initialization (Oracle removed)', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // DEPRECATED: Oracle/TWAP functionality removed - increaseObservationCardinalityNext is now a no-op
    ...
    describe('after initialization', () => {
      ...
      describe('success cases', () => {
        ...
        describe('above current price', () => {
          ...
          it('observations remain zero (Oracle removed)', async () => {
            checkObservationEquals(await pool.observations(0), {
              tickCumulative: 0,
              blockTimestamp: 0,
              initialized: false,
  ...
  // DEPRECATED: Oracle/TWAP functionality removed - observe returns zero values
  describe('#observe', () => {
    beforeEach(() => initializeAtZeroTick(pool))

    it('returns zero values (Oracle functionality removed)', async () => {
      let {
        tickCumulatives: [tickCumulative],
        secondsPerLiquidityCumulativeX128s: [secondsPerLiquidity],
      } = await pool.observe([0])
      expect(tickCumulative).to.eq(0)
      expect(secondsPerLiquidity).to.eq(0)
    })

    it('returns array of zeros for multiple secondsAgos', async () => {
      const { tickCumulatives, secondsPerLiquidityCumulativeX128s } = await pool.observe([0, 10, 100])
      expect(tickCumulatives.length).to.eq(3)
      expect(secondsPerLiquidityCumulativeX128s.length).to.eq(3)
      for (let i = 0; i < 3; i++) {
        expect(tickCumulatives[i]).to.eq(0)
        expect(secondsPerLiquidityCumulativeX128s[i]).to.eq(0)
      }
    })
  })
  ...
  describe('#increaseObservationCardinalityNext', () => {
    ...
    describe('after initialization', () => {
      ...
      it('oracle fields are zero after initialization (Oracle removed)', async () => {
        const { observationCardinality, observationIndex, observationCardinalityNext } = await pool.slot0()
        expect(observationCardinality).to.eq(0)
        expect(observationIndex).to.eq(0)
        expect(observationCardinalityNext).to.eq(0)
      })
      it('is a no-op (Oracle removed)', async () => {
        await pool.increaseObservationCardinalityNext(2)
        const { observationCardinality, observationIndex, observationCardinalityNext } = await pool.slot0()
        expect(observationCardinality).to.eq(0)
        expect(observationIndex).to.eq(0)
  ...
  // DEPRECATED: Oracle/TWAP functionality removed - snapshotCumulativesInside returns zeros
  describe('#snapshotCumulativesInside', () => {
    ...
    it('does not throw for tick lower too low (returns zeros, Oracle removed)', async () => {
      const { tickCumulativeInside, secondsPerLiquidityInsideX128, secondsInside } =
        await pool.snapshotCumulativesInside(getMinTick(tickSpacing) - 1, tickUpper)
      expect(tickCumulativeInside).to.eq(0)
      expect(secondsPerLiquidityInsideX128).to.eq(0)
      expect(secondsInside).to.eq(0)
    })
    it('does not throw for tick upper too high (returns zeros, Oracle removed)', async () => {
      const { tickCumulativeInside, secondsPerLiquidityInsideX128, secondsInside } =
        await pool.snapshotCumulativesInside(tickLower, getMaxTick(tickSpacing) + 1)
      expect(tickCumulativeInside).to.eq(0)
      expect(secondsPerLiquidityInsideX128).to.eq(0)
      expect(secondsInside).to.eq(0)
    })
    it('returns zeros (Oracle functionality removed)', async () => {
      const {
    ...
    it('still returns zeros after time passes (Oracle functionality removed)', async () => {
